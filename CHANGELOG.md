...
### Added
- **Pre-Launch Handover generation** — the Skool_Skale_DFY machine baked into the builder. A "Generate Handover" section on the export page (available once every module is approved) enqueues an Inngest run that produces the canonical 6-file client package: `00-README` (deterministic index + `[[placeholder]]` checklist), `01-vsl-and-cancellation` (the long VSL doubles as the About-Us page script), `02-pre-launch-emails` (guest-speaker sequence only when the VA explicitly checks the confirmation box — never inferred from `guest_sessions`), `03-post-launch-emails`, `04-docuseries-full-script`, and `05-dm-sequences`. Copy generation runs on `claude-opus-4-8` via the raw `@anthropic-ai/sdk` (new dep) with Anthropic prompt caching: the five deliverable calls share a byte-identical system prefix (role + voice-and-guardrails + handover structure + brief + full DNA markdown from `serializePackageMarkdown`), deliverable 01 fires alone to write the cache, the remaining four read it concurrently at ~10% input price (~$1/package measured on the DFY CLI). Frameworks + gold examples live as 13 verbatim markdown files under `src/prompts/handover/assets/` (traced into the Vercel function via `outputFileTracingIncludes`); the DFY brief/parse/scan/readme logic is ported to pure TS modules in `src/prompts/handover/`. Each doc is rendered to a styled PDF inside the same Inngest run via `puppeteer-core` + `@sparticuz/chromium` (new deps; local dev needs `PUPPETEER_EXECUTABLE_PATH` pointing at an installed Chrome/Edge — the sparticuz binary is Linux-only) using a TS port of the DFY Notion-styled markdown→HTML converter, uploaded to the new private `handover-docs` bucket, and downloaded per-doc through signed-URL redirect + markdown-attachment routes. New tables `handover_runs` (pollable run state + aggregate usage incl. cache token classes; migration `0009`) and `handover_documents` (versioned per package+docKey, content + counts + pdfPath) with the standard workspace RLS block; deliverable keys are deliberately NOT `module` enum values so export gating and module routes stay untouched. `estimateCostUsd` gains cache read/write token classes and `claude-opus-4-8` rates; the handover finalize step becomes the first writer of `launch_packages.total_cost_usd`. **Manual steps at merge time:** `npx dotenv -e .env.local -- drizzle-kit migrate` (verify `select unnest(enum_range(NULL::handover_doc_key));`) and `ANTHROPIC_API_KEY=placeholder pnpm storage:setup` + `pnpm verify:bucket` for the new bucket.

### Removed
- **AI image generation removed entirely.** Skool Skale decided to drop AI-generated visuals; VAs now produce community icon, community cover, classroom cover, and calendar cover externally in Canva using the client's professional photography. The builder becomes a copy-only generator. Cut from the registry, the Inngest pipeline, the dashboard, the export view, and the deployment checklist: the four image modules (`cover`, `icon`, `classroom_cover`, `calendar_cover`), the `gemini-image` generator kind, the `image-variants` and `image-single` card variants, the `hasVariants` / `showEdit` / `fullWidth` ModuleConfig fields, the `GOOGLE_AI_API_KEY` env var, the `@google/genai` package, the `src/lib/gemini-image/` integration, the `src/lib/image-providers/` directory, the `src/lib/storage/module-buckets.ts` + `resolve-variants.ts` helpers, the `select-variant` route, and the image download-redirect route. The DB schema is untouched per the cut-then-cleanup convention: `generation_jobs.gemini_image_usage` column, the `cover-variants` and `image-variants` Supabase Storage buckets, and any existing `module='cover'/'icon'/...` rows in `generated_assets` are intentionally orphaned — live client deployments still resolve the URLs they were given, but the registry no longer surfaces those rows in the dashboard or export view. Active module count drops from 13 to 9; "X of Y modules approved" math reflects 9. Package generation drops the longest-running step (Gemini variants) and one of the two API spend lines — wall-clock is materially shorter. **Manual post-merge step:** `GOOGLE_AI_API_KEY` must be removed from Vercel via CLI (`vercel env rm GOOGLE_AI_API_KEY`); the dashboard can't widen/delete sensitive env vars. The `cover-variants` + `image-variants` Supabase buckets stay in place — removal is a separate ops decision once the legacy URLs in deployed Skool communities are confirmed retired. Signed-URLs migration tech debt items now N/A.

### Changed
- Signed-URLs migration **Stage 4 of 4** — `cover-variants` and `image-variants` Supabase Storage buckets flipped from public to private. `scripts/setup-storage.ts` gains a per-bucket `public` flag (`false` for both Gemini-output buckets; `creator-photos` stays `true` pending a follow-up that migrates the upload component to signed URLs). `scripts/verify-buckets.ts` now asserts the live `public` flag matches `EXPECTED[].public` rather than blanket-requiring `public=true`. The four image writers (`generate-cover.ts`, `generate-icon.ts`, `generate-classroom-cover.ts`, `generate-calendar-cover.ts`) drop their `getPublicUrl` calls and persist `url: ""` — `storagePath` is the sole source of truth, and the Stage 3 reader (`resolveAssetUrls`, PR #27) rewrites `url` with a fresh 1-hour signed URL on every page render. Writer unit tests drop the `getPublicUrl` mock entirely so any regression that re-introduces the call surfaces as a `TypeError` instead of silently re-persisting a public URL. **Manual step at merge time:** run `pnpm storage:setup` against prod to apply the bucket visibility flip; `pnpm verify:bucket` confirms the final state.
- Intake cleanup — Step 2 ("The offer") drops the standalone `courses` and `events` fields. Both were redundant: classroom titles already live on the Step 5 `classroom_titles` repeater, and live events already live on the Step 5 `calendar_intake.events` repeater (PR #31). `CreatorIntakeSchema.offer_breakdown` now carries only `perks` + `guest_sessions`. The classroom prompt drops its `<courses>` context block, and the discovery_seo prompt's offer hints now derive from `classroom_titles` + `calendar_intake.events[].title` instead of the deprecated free-form strings. Legacy DB rows with `courses` / `events` keys parse cleanly — Zod's default object behavior strips them on hydration.
- Intake cleanup — Step 3 ("Pricing & terms") locks the additional-tier structure to Skool's real model. `pricing.tiers` renamed to `pricing.additional_tiers`; tier name becomes a `z.enum(['Premium', 'VIP'])` literal union, capped at 2 rows with a schema refine enforcing Premium-first / VIP-second ordering. UI renders the name as a static label (no input), only price is editable. The "Add tier" button label tracks the next available slot — "Add Premium tier" → "Add VIP tier" → hidden. Removing Premium cascade-removes VIP to preserve the order invariant; the wizard surfaces an inline hint about this when a Premium row exists alone.
- Categories module drops the `description` field everywhere. Skool's category UI is name-only, so generating descriptions was wasted Claude tokens, dead weight in the dashboard, and confusing on export. `CategoriesContentSchema` is now `{categories: tuple-of-3 non-empty strings}` (was tuple-of-3 `{name, description}` objects); same shape as `LeaderboardContentSchema.levels`. Wizard step 5 switches the categories repeater from `variant="grouped"` to `variant="single"`; the generator prompt + parser only handle `<name>` tags now (descriptions in model output are rejected, keeping prompt and parser in lock-step). Dashboard card, ExportView section, and edit dialog all updated to the string-list shape.
