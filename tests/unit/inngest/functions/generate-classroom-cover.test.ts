 ...
 *
 * Single-variant twin of generate-cover.test.ts. Identical shape,
 * different module key + builder.
 */
...
vi.mock("@/prompts/classroom_cover", () => ({
  buildClassroomCoverPrompt: buildClassroomCoverPromptMock,
}));
...
import { generateClassroomCover } from "@/lib/inngest/functions/generate-classroom-cover";
...
async function runHandler(data: EventData) {
  const handler = generateClassroomCover as unknown as (ctx: {
...
beforeEach(() => {
  vi.clearAllMocks();
  buildClassroomCoverPromptMock.mockReturnValue("BUILT BY THE BUILDER");
...
describe("generateClassroomCover editedPrompt branch", () => {
  test("WITHOUT editedPrompt: builder called once; image provider receives builder output", async () => {
    ...
    expect(buildClassroomCoverPromptMock).toHaveBeenCalledTimes(1);
  ...
  test("WITH editedPrompt: builder NOT called; image provider receives edited string", async () => {
    ...
    expect(buildClassroomCoverPromptMock).not.toHaveBeenCalled();
    expect(imageProviderGenerateMock).toHaveBeenCalledTimes(1);
    expect(imageProviderGenerateMock.mock.calls[0][0].prompt).toBe(
