<!--
Vendored from https://github.com/blader/humanizer (SKILL.md, v3.0.0), MIT licensed.
Upstream derives its patterns from Wikipedia's "Signs of AI writing" (WikiProject
AI Cleanup). Kept as plain markdown so it stays reviewable and diffable against
upstream; see LICENSE-humanizer.md for the full license text.

LOCAL EDITS: none. Handover-specific carve-outs (headings the parser depends on,
[[placeholder]] tokens, merge tags, CTA rows) live in humanize.ts as a separate
preservation contract rather than being edited into this file, so upstream
updates stay a clean copy-over.
-->
